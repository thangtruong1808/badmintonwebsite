...
/**
 * @route   POST /api/upload/service-flyer
 * @desc    Upload service flyer image to Cloudinary (admin only)
 * @access  Private (admin)
 */
router.post(
  '/service-flyer',
  ...
  async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const uploadStream = cloudinary.uploader.upload_stream(
        {
          folder: 'chibibadminton/service-flyer',
...
/**
 * @route   POST /api/upload/key-person-image
 * @desc    Upload key person (team/leadership) photo to Cloudinary (admin only).
 * @access  Private (admin)
 */
router.post(
  '/key-person-image',
  ...
  async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const uploadStream = cloudinary.uploader.upload_stream(
        {
          folder: 'chibibadminton/key-persons',
          public_id: `keyperson_${Date.now()}`,
          transformation: [
            { width: 400, height: 400, crop: 'fill', gravity: 'face' },
            { quality: 'auto:good' },
          ],
