...
const EventCard: React.FC<EventCardProps> = ({
  ...
  const getStatusBadge = () => {
    if (isCancelled) {
      return (
        <span className="bg-red-600 text-white text-xs font-bold px-2 py-1 rounded font-calibri">
          CANCELLED
        </span>
      );
    }
    if (isCompleted) {
      return (
        <span className="bg-gray-600 text-white text-xs font-bold px-2 py-1 rounded font-calibri">
          COMPLETED
        </span>
      );
    }
    if (isFull) {
      return (
        <span className="bg-orange-500 text-white text-xs font-bold px-2 py-1 rounded font-calibri">
          FULL
        </span>
      );
    }
    return (
      <span className="bg-green-500 text-white text-xs font-bold px-2 py-1 rounded font-calibri">
        AVAILABLE
      </span>
    );
  };
  ...
  return (
    <div
      className={`bg-white rounded-xl shadow-lg overflow-hidden flex flex-col transition-all duration-300 hover:shadow-xl ${isSelected
        ? "ring-4 ring-rose-500 border-2 border-rose-500 font-calibri"
      ...
      <div className="p-4 flex-grow flex flex-col relative">
          ...
          <div className="flex items-center gap-2 flex-shrink-0 font-calibri">
            ...
            {!isCompleted && !isCancelled && (
              <input
                ...
                className="w-5 h-5 rounded border-gray-300 text-rose-500 focus:ring-rose-500 cursor-pointer font-calibri"
        ...
        <div className="flex gap-2 mt-auto">
          {!isCompleted && !isCancelled && (
            <button
              ...
              className={`flex-1 py-2 px-4 rounded-lg font-semibold text-sm transition-colors ${isFull
                ? "bg-gray-300 text-gray-500 cursor-not-allowed font-calibri"
                : "bg-rose-500 text-white hover:bg-rose-600 font-calibri"
                }`}
            >
          ...
          <button
            ...
            className="flex-1 py-2 px-4 rounded-lg font-semibold text-sm bg-gray-200 text-gray-700 hover:bg-gray-300 transition-colors font-calibri"
          >
            Details
          </button>
        </div>
